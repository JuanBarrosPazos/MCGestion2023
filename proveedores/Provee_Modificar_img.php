...
function process_form(){
	...
	$sqla = "UPDATE `$db_name`.$gst_proveedores SET `myimg` = '$new_name'  WHERE $gst_proveedores.`id` = '$id' LIMIT 1 ";
		
	$sqlax = "UPDATE `$db_name`.$gst_proveedoresfeed SET `myimg` = '$new_name'  WHERE $gst_proveedoresfeed.`dni` = '$dinold' LIMIT 1 ";

	if(mysqli_query($db, $sqlax)){print("TODO VA BIEN");}else {
...
global $text;
$text = "- PROVEEDORES MODIF IMG".$ActionTime.". ID:".$_POST['id'].". ".$nombre." ".$apellido." / ".$destination_file;

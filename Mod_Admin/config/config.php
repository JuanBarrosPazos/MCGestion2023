...
function inittot(){
	...
	if(($inst == 1)&&(@mysqli_num_rows(mysqli_query($db, $sqltadm)) < 1)){
		...
		global $text;
		$text ="FORMULARIO EXISTE UNA INSTALACION INCOMPLETA";
		ini_log();
		...
		global $text;
		$text ="!!! FORMULARIO EXISTE UNA INSTALACION ANTERIOR";
		ini_log();
...
function validate_form(){
	...
	require 'validate_Init_System.php';
...
////////////////////				////////////////////				////////////////////
	...
	if (file_exists($carpeta)) {
		copy("index.php", $carpeta."/index.php");
	...
	if (file_exists($carpetat)) {
		copy("SecureIndex2.php", $carpetat."/index.php");
		$data0 = $data0."\t* OK SECURE INDEX.PHP".PHP_EOL;;
	} else {}

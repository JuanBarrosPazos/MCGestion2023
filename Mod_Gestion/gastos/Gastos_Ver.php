...
function process_form(){
	...
	$sqlc =  "SELECT * FROM $gastos WHERE `factnum` = '$fnum' AND `factdate` LIKE '$fil' OR `factnif` = '$fnif' AND  `factdate` LIKE '$fil' OR `factnom` LIKE '$fnom' AND  `factdate` LIKE '$fil' ORDER BY $orden ";
...
<form name='ver' action='Gastos_Ver_02.php' target='popup' method='POST' onsubmit=\"window.open('', 'popup', 'width=440px,height=670px')\">
	...
	<input name='vname' type='hidden' value='".$gastos."' />
...
function ver_todo(){
	...
	require "../config/TablesNames.php";
	$sqlb =  "SELECT * FROM $gastos WHERE `factdate` LIKE '$fil' ORDER BY $orden ";

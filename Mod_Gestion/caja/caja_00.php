...
if(strlen($_POST['cnivela']) == 0){
	require "../config/TablesNames.php";
	$sqlc =  "SELECT * FROM $clientes WHERE `Nombre` LIKE '$nom' OR `Apellidos` LIKE '$ape' OR `ref` LIKE '$refclient'  ORDER BY `Nombre` ASC  ";
...
elseif(strlen($_POST['cnivela']) != 0){
	require "../config/TablesNames.php";
	$sqlc =  "SELECT * FROM $admin WHERE `Nombre` LIKE '$nom' OR `Apellidos` LIKE '$ape' OR `ref` LIKE '$refclient'  ORDER BY `Nombre` ASC  ";
...
$_SESSION['nclient'] = $_POST['Nivel'];
	...
	$sql = "UPDATE `$db_name`.$caja SET `refclient` = '$_POST[ref]', `clname` = '$clname' WHERE `oper` = '$oper' ";
...
function recup_compra(){
	...
	require "../config/TablesNames.php";
	$rc =  "SELECT * FROM $caja  ORDER BY `oper` ASC ";
	$qrc = mysqli_query($db, $rc);
	$count = mysqli_num_rows($qrc);
...
if (( $rowrc['oper'] == $oper ) && ( $rowrc['kgcash'] == 00.00 )){
	...
	$sql = "UPDATE `$db_name`.$caja SET `refclient` = '$refclient', `vseccion` = '$_POST[seccion]', `producto` = '$_POST[valor]', `proname` = '$_POST[proname]', `kgcash` = '$kgcash', `psiva` = '$_POST[psiva]', `iva` = '$_POST[iva]', `ivae` = '$ivav', `pvp` = '$_POST[pvp]', `pvptot` = '$pvptot' WHERE `oper` = '$oper' AND `kgcash` = '00.00'  LIMIT 1 ";
	...
	$ic = "INSERT INTO `$db_name`.$caja (`cname`, `refcaja`, `oper`, `nsemana`, `datecash`) VALUES ('$rowrc[cname]', '$refcaja', '$rowrc[oper]', '$rowrc[nsemana]', '$rowrc[datecash]')";

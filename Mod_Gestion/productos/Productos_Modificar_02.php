...
/********** MODIFICA LA VARIABLE Y NOMBRE DEL PRODUCTO EN LA TABLA CAJA	***********/
	...
	$cj =  "SELECT * FROM `$db_name`.$caja WHERE $caja.`producto` = '$_POST[valor1]'";
	...
	$sqlmod = "UPDATE `$db_name`.$caja SET `producto` = '$valormod', `proname` = '$_POST[nombre]' WHERE $caja.`producto` LIKE '$filmod' ";

	if(mysqli_query($db, $sqlmod)){ print("</br>
			HA MODIFICADO LAS VARIABLES EN ".$caja.": ".$_POST['valor1']." POR L392".$_POST['valor2'].".");
		} else { print("<font color='#FF0000'>
				 * </font>&nbsp;&nbsp;".mysqli_error($db))."
				 </br>";
			}

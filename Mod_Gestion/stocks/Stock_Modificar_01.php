...
if(strlen($_POST['vsolo']) != 0){
	require "../config/TablesNames.php";
	$sqlc = "SELECT * FROM $tablastock3 WHERE `producto` LIKE '$fil' AND $camp2 LIKE '$fil2' AND `kgdifer` > '0' ORDER BY $orden";
}
...
if(strlen($_POST['tcomen']) == 0){$tcomen = substr($rowc['coment'],-76);}
else{$tcomen = $rowc['coment'];}
...
global $coment;
$coment = $rowc['coment']."</br>* Modifc: ".$rowc['kgin']."-(".$rowc['kgbad']."+".$rowc['kgcash'].")=".$rowc['kgdifer'];

			print (	"<tr align='center'>
									
				<form name='modifica' action='Stock_Modificar_02.php' method='POST'>
		...
		<input name='coment' type='hidden' value='".$coment."' />".$tcomen."
						</td>
...
$coment = $_POST['coment']."</br>".$rowc['kgin']."-(".$rowc['kgbad']."+".$rowc['kgcash'].")=".$rowc['kgdifer']."</br> Añadidas ".$rowc['kgdifer']." Unit de la entrada, ID: ".$rowc['id'];
						} 
...
function accion_modifica_01(){
	...
	if (($_SESSION['Nivel'] == 'admin') || ($_SESSION['Nivel'] == 'plus')){ $dir = 'Admin';}
		...
		global $text;
		$text = "- STOCK MODIFICAR 1 ".$ActionTime.". ".$secc.$pro.$value;

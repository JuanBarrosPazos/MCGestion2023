...
function recup_compra(){
	...
	require "../config/TablesNames.php";
	$rc =  "SELECT * FROM $gst_caja WHERE `refclient` = '$ref' ORDER BY `oper` ASC ";
	$qrc = mysqli_query($db, $rc);
	$count = mysqli_num_rows($qrc);
	...
	require "../config/TablesNames.php";
	$dtcl =  "SELECT * FROM $gst_clientes WHERE `ref` = '$ref' ";
	$qdtcl = mysqli_query($db, $dtcl);
	$h = 'height=620px';
...
<form name='ver' action='Productos_Ver_02.php' target='popup' method='POST' onsubmit=\"window.open('', 'popup', 'width=440px,height=640px')\">
	...
	$ncl =  "SELECT * FROM $gst_clientes WHERE `ref` = '$ref' ";
	$qncl = mysqli_query($db, $ncl);
	$rowncl = mysqli_fetch_assoc($qncl);
	$_SESSION['nclient'] = $rowncl['Nivel'];
	// print(" ".$_SESSION['nclient']);
...
if (( $rowrc['oper'] == $oper ) && ( $rowrc['kgcash'] == 00.00 )){
	...
	$sql = "UPDATE `$db_name`.$gst_caja SET `clname` = '$cajana', `refclient` = '$refclient', `vseccion` = '$_POST[seccion]', `producto` = '$_POST[valor]', `proname` = '$_POST[proname]', `kgcash` = '$kgcash', `psiva` = '$_POST[psiva]', `iva` = '$_POST[iva]', `ivae` = '$ivav', `pvp` = '$_POST[pvp]', `pvptot` = '$pvptot' WHERE `oper` = '$oper' AND `kgcash` = '00.00'  LIMIT 1 ";
	...
	$ic = "INSERT INTO `$db_name`.$gst_caja (`cname`, `refcaja`, `oper`, `nsemana`, `datecash`) VALUES ('$cajana', '$refcaja', '$rowrc[oper]', '$rowrc[nsemana]', '$rowrc[datecash]')";

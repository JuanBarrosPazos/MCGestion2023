...
/* 	VALIDAMOS EL CAMPO my_img */
	...
	$prove =  "SELECT * FROM `$db_name`.$gst_proveedores WHERE `ref` = '$rf'";
	$qprove = mysqli_query($db, $prove);
	$cprove = mysqli_num_rows($qprove);
		
	if($cprove > 0){$errors [] = "YA EXISTE EL PROVEEDOR ".$rf;}
						
	elseif($_FILES['myimg']['size'] == 0){}
...
/*		if($_FILES['myimg']['size'] == 0){
	...
	elseif (!preg_match('/^[^0-9@#$%<>:"·\(\)=¿?!¡\[\]\{\};,:\.\*]+$/',$_POST['rsocial'])){
		$errors [] = "R. SOCIAL <font color='#FF0000'>Solo se admite texto</font>";
		}

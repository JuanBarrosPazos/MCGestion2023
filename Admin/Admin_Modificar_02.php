...
/*******************************/
	...
	$sqlc = "UPDATE `$db_name`.`admin` SET `ref` = '$rf',`Nivel` = '$_POST[Nivel]', `Nombre` = '$_POST[Nombre]', `Apellidos` = '$_POST[Apellidos]', `doc` = '$_POST[doc]', `dni` = '$_POST[dni]', `ldni` = '$_POST[ldni]', `Email` = '$_POST[Email]', `Usuario` = '$_POST[Usuario]', `Password` = '$_POST[Password]', `Direccion` = '$_POST[Direccion]', `Tlf1` = '$_POST[Tlf1]', `Tlf2` = '$_POST[Tlf2]' WHERE `admin`.`ID` = '$_POST[ID]' LIMIT 1 ";

	if(mysqli_query($db, $sqlc)){
							print( $tabla );
				} else {
				print("<font color='#FF0000'>
						* MODIFIQUE LA ENTRADA: </font>
						</br>
						&nbsp;&nbsp;&nbsp;".mysqli_error($db))."
						</br>";
						show_form ();
	...
	$Year2 = $Year - 1;
	$ventas = 'ventas_'.$Year;
	$ventas2 ='ventas_'.$Year2;

$sqlcv = "UPDATE `$db_name`.`$ventas` SET `refclient` = '$rf' WHERE `$ventas`.`refclient` = '$srefcl' ";
...
$sqlcv2 = "UPDATE `$db_name`.`$ventas2` SET `refclient` = '$rf' WHERE `$ventas2`.`refclient` = '$srefcl' ";
